...
class WakeWordService {
  ...
  Future<void> _startPorcupine(String? accessKey, String? keywordPath) async {
    // Porcupine est disponible comme package Flutter
    // Import conditionnel pour éviter les erreurs sur plateformes non supportées
    if (Platform.isAndroid || Platform.isIOS || Platform.isMacOS || Platform.isLinux || Platform.isWindows) {
      // TODO: Implémenter Porcupine avec le package `porcupine`
      // final porcupine = await Porcupine.create(
      //   accessKey: accessKey ?? '',
      //   keywordPaths: keywordPath != null ? [keywordPath] : null,
      //   onDetection: () => onWakeDetected?.call(),
      // );
      // _porcupineSubscription = porcupine.audioStream?.listen(...);
      _isActive = true;
    } else {
      _isActive = true;
    }
  ...
  Future<bool> _requestMicrophonePermission() async {
    try {
      // Sur mobile, la permission micro est gérée par le système
      // speech_to_text gère déjà cette demande
      return true;
    } catch (_) {
      return false;
    }
...
class MicrophonePermission {
  static Future<bool> request() async {
    try {
      // speech_to_text gère les permissions nativement
      return true;
    } catch (_) {
      return false;
    }
